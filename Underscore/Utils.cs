...
namespace Underscore
{
    public class Utils
    {
        private static readonly string[] CommandStartCharacters = { "--", "-" };
        ...
        public static Arguments GetCommandLineArguments(string[] args)
        {
            ...
            {
                var found = false;
                foreach (var startCharacter in CommandStartCharacters)
                {
                    if (args[i].StartsWith(startCharacter))
                    {
                        found = true;
                        string command = args[i].Remove(0, startCharacter.Length);
                        string value = string.Empty;

                        if (i < args.Length - 1 && (!args[i + 1].StartsWith(CommandStartCharacters[0]) && !args[i + 1].StartsWith(CommandStartCharacters[1])))
                        {
                            value = args[i + 1];
                            i++;
                        }
                        ...
                        {
                            Console.WriteLine("Duplicate command line argument " + command);
                        }
                    }
                }
            ...
            return new Arguments(commandToValueDictionary, commandList, ArgumentConstants.Verbose);
        }
        ...
        public static void AssertRequiredArguments<T>(Arguments args, bool strict)
        {
            ...
            Debug.Assert(invalidValues.Count == 0, string.Join(Environment.NewLine, invalidValues.ToArray()));
        }
        ...
        public static string GetArgumentsDescription(Type type, int spaces = 0)
        {
            ...
            foreach (var info in consts)
            {
                ...
                if (attr != null)
                {
                    ...
                    builder.AppendFormat(CommandStartCharacters[0] + "{0} {1} {4} -- {3} {2} ", info.GetValue(type),
